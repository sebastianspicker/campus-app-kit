...
export function parseIcs(ics: string, options?: ParseIcsOptions): ParsedIcsEvent[] {
  ...
  const context = parseContext(options, countRecurringEvents(ics, {
    referenceDate: limits.referenceDate,
    horizonDays: limits.horizonDays,
    maxInstances: limits.expansionMaxInstances
  }));
