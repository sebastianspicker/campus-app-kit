...
import { guardAuth, isInvalidAuthAttempt } from "./auth";
...
describe("guardAuth", () => {
  ...
  it("requires the configured bearer token when auth is enabled", () => {
    ...
    expect(isInvalidAuthAttempt(allowed.request)).toBe(false);

    const prefix = createMockReqRes({ headers: { authorization: "Bearer expected" } });
    expect(guardAuth(prefix.request, prefix.response)).toBe(false);
    expect(isInvalidAuthAttempt(prefix.request)).toBe(true);
  });
